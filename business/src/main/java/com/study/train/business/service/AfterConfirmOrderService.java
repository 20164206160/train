...
import com.study.train.business.mapper.cust.DailyTrainTicketMapperCust;
...
public class AfterConfirmOrderService {
    ...
    public void afterDoConfirm(DailyTrainTicket dailyTrainTicket, List<DailyTrainSeat> finalSeatList, List<ConfirmOrderTicketReq> tickets, ConfirmOrder confirmOrder) throws Exception {
         ...
         for (int j = 0; j < finalSeatList.size(); j++) {
            ...
            DailyTrainSeat updateDailyTrainSeat = new DailyTrainSeat();
            updateDailyTrainSeat.setId(dailyTrainSeat.getId());
            updateDailyTrainSeat.setSell(dailyTrainSeat.getSell());
            updateDailyTrainSeat.setUpdateTime(new Date());
            dailyTrainSeatMapper.updateByPrimaryKeySelective(updateDailyTrainSeat);
            ...
            char[] chars = updateDailyTrainSeat.getSell().toCharArray();
            Integer maxStartIndex = endIndex - 1;
            Integer minEndIndex = startIndex + 1;
            ...
            Integer maxEndIndex = updateDailyTrainSeat.getSell().length();
            for(int i= endIndex; i < updateDailyTrainSeat.getSell().length(); i++) {
                char aChar = chars[i];
                if (aChar == '1') {
                    maxEndIndex = i;
                    break;
